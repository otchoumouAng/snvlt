 ...
 */
class TypeDocumentController_ extends AbstractController
{
    /**
    ...
    #[Route('/CategoriesActes/list', name: 'getCategoriesActes')]
    public function getCategoriesActes(
    ...
    ): Response
    {
        ...
        $categories =$registry->getRepository(TypeDemande::class)->findAll();
    ...
    #[Route('/CategoriesActesByid/{id_categorie}', name: 'getCategoriesActesById')]
    public function getCategoriesActesById(
        ManagerRegistry $registry,
        int $id_categorie,
        Request $request
    ): Response
    {
        ...
        if(!$request->getSession()->has('user_session') or !$this->getUser()){
            ...
            $categorie =$registry->getRepository(TypeDemande::class)->find($id_categorie);

            if($categorie){
                $actes =$registry->getRepository(CatalogueServices::class)->findBy(['categorie_activite'=>$categorie]);
                foreach($actes as $acte) {
                    $reponse[] = array(
                        'designation'=>$acte->getDesignation(),
                        'id'=>$acte->getId()
                    );
                }
            }
